...
test("Apple live program titles and broadcast times are not subtitle cues", () => {
  const metadata = element("DIV", { class: "video-metadata" });
  const title = element("SPAN", { class: "subtitle-text" }, metadata);
  assert.equal(isSubtitleElement(title), false);
});
